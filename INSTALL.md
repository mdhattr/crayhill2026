...
- **`GET /api/v1/health` returns HTTP 500 with `AH00124: Request exceeded the limit of 10 internal redirects` in `/var/log/httpd/crayhill_error.log`.** The Apache rewrite in `config/httpd/crayhill.conf` was re-appending `.php` on every pass (`health` → `health.php` → `health.php.php` → …). Pull the latest repo and re-run `bash scripts/setup-api-ec2.sh` (the vhost now includes `RewriteCond %{REQUEST_FILENAME} !-f` before the rule). Quick manual fix on the box — edit `/etc/httpd/conf.d/crayhill.conf` and add that line immediately above the `RewriteRule ^v1/` line, then `sudo apachectl configtest && sudo systemctl reload httpd`.
